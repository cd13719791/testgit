...
public abstract class InterceptorHookSupport implements InterceptorHook {
	 ...
	 * <p>
	 * 其中IndexControllerInterceptorListener是继承InterceptorListener的接口，
	 * 拥有区分一类钩子下的所有插件
	 */
	@PostConstruct
	...
	protected void onPostHandle(HttpServletRequest request, HttpServletResponse response, HandlerMethod handler, ModelAndView modelAndView)
			throws Exception {
		Iterator<Map.Entry<String, InterceptorListener>> it = this.plugins.entrySet().iterator();
		while (it.hasNext()) {
